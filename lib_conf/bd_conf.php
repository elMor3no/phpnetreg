<?php
/*! Medoo 0.9.8 - Copyright 2015, Angel Lai - MIT license - http://medoo.in */
class medoo
{
      ...
      try
      {
         ...
         switch($type)
         {
            ...
            case 'pgsql':
               ...
               break;
            case 'sybase':
               $dsn='dblib:host='.$this->server.($is_port?':'.$port:'').';
               dbname='.$this->database_name;
               break;
            case 'oracle':
               ...
               break;
            case 'mssql':
               $dsn=strstr(PHP_OS,'WIN')?'sqlsrv:server='.$this->server.($is_port?','.$port:'').';
               database='.$this->database_name:'dblib:host='.$this->server.($is_port?':'.$port:'').';
               dbname='.$this->database_name;
